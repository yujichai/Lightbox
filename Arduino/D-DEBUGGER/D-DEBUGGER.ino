...
void loop() 
{
  if (Serial.available() > 0) {
    ...
    if(incomingByte == -35){
      ...
      if(digit == 0){
        selection = incomingByte;
      }
      if((selection == 1)&&(digit == 2)){
        selection = 10;
      ...
      //command = incomingByte % 10;
      //selection = (incomingByte - command) / 10;
      if(flag == 1){
        flag = 0;
        if((selection >= 1)&&(selection <= 10)&&(command >= 0)&&(command <= 3)){
        tx_buf[0] = selection + 240;
        tx_buf[1] = command;
