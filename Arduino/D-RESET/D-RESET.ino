...
void setup() 
{
  ...
  pinMode(CE,  OUTPUT);
  pinMode(CSN, OUTPUT);
  pinMode(re_over, OUTPUT);
  pinMode(re_till, OUTPUT);
...
void loop() 
{
  ...
  unsigned char status = SPI_Read(STATUS);                         // read register STATUS's value
    if(status&RX_DR)                                                 // if receive data ready (TX_DS) interrupt
    {
      ...
      unsigned char command=rx_buf[1];  
      SPI_RW_Reg(FLUSH_RX,0);
      Serial.print(command); 
      Serial.print(" \n");
      if(command==2)
      {
        digitalWrite(re_over,HIGH);
        digitalWrite(re_till,LOW);
        Serial.write(2);
      }
      if(command==3)
      {
        digitalWrite(re_over,LOW);
        digitalWrite(re_till,HIGH);  
        Serial.write(3);
      }
    }
